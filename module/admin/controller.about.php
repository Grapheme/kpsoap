<?
class about_admin extends TAdminController {
	public function about($code_name){
		$this->content = $this->load_module($code_name);
		
		if (action == NULL) {###

			$content = $this->core->sql->query("SELECT * FROM `dat_about` WHERE `alias`='".(empty($this->page)?'_':$this->page)."' LIMIT 0,1");
			...
			$this->core->sql->update('dat_about', array('alias', 'priority', 'title', 'menutitle', 'text'),
			...
			$content = $this->core->sql->query("SELECT max(`priority`) as `priority` FROM `dat_about`"); //`priority` as MAX(
			...
			//echo $this->core->sql->get_next_auto_increment('dat_about');

		} elseif (action == 'insertgroup') {###

			$this->core->sql->insert('dat_about', array('alias', 'priority', 'title', 'menutitle', 'text'),	
													  array($_POST['alias'], $_POST['priority'], $_POST['title'], $_POST['menutitle'], $_POST['text']));
			header(func::url_delget("location: http://{$_SERVER['SERVER_NAME']}{$_SERVER['REQUEST_URI']}"));
			...
			$this->core->sql->delete('page_panels', "`alias` = '{$code_name}'");
			$this->core->sql->delete('page_client', "`alias` = '{$code_name}'");
			$this->core->sql->delete('dat_about', "`alias` = '{$this->page}'");
			header(func::url_delget("location: http://{$_SERVER['SERVER_NAME']}{$_SERVER['REQUEST_URI']}"));
		}
