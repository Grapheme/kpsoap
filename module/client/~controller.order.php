			...
			if ($order->num_rows>0) {
				...
				if (!empty($client->e_mail)) {
					...
					$text = "<h2>Статус заказа №{$id} изменился.</h2>
							 <p>Ужаваемый(ая), {$name}. Ваш заказ №{$id} оплачен.</p>";
					$info = "<p>Это письмо отправлено автоматически, и Вам не следует на него отвечать. Для получения более подробной информации Вы так же можете обратиться к нам по номеру 8 (800) 775-25-10 (звонок беспратный).</p>
							 <p>Благорабим Вас за выбор нашего магазина.</p>";
				...
				$text = "<h2>{$title}</h2>
						 <p>{$name} произвел(а) оплату заказа №{$id}</p>"; 
			...
			if (isset($_POST['secret']) && @$_POST['secret'] == $_SESSION['secret']) {
				...
				if (!empty($_POST['e_mail'])) {
					...
					$info = "<p>&nbsp;</p>
							 ...
							 <p>Благорабим Вас за выбор нашего магазина.</p>"; //http://{$_SERVER['SERVER_NAME']}/order/:pay-{$payhash}
